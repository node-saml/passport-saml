...
SAML.prototype.initialize = function (options) {
  ...
  if (!options.recipient) {
    // By the SAML 2.0 profiles specification, should be equal to the ACS URL.
    options.recipient = options.callbackUrl;
  }
  ...
  if(!options.maxAssertionAgeMs) {
    // Default to no max age, time counted in milliseconds.
    options.maxAssertionAgeMs = 0;
  }

  if(!options.nowInMs) {
    options.nowInMs = function() {
...
SAML.prototype.generateInstant = function () {
  return new Date(this.options.nowInMs()).toISOString();
};
...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, callback) {
  ...
  .then(function(){
    ...
    // Note we don't add a samlp:RequestedAuthnContext because it's generally
    // not necessary and ADFS is very picky about parsing it, often barfing.
...
SAML.prototype.requestToUrl = function (request, response, operation, additionalParameters, callback) {
  ...
  zlib.deflateRaw(request || response, function(err, buffer) {
    ...
    var targetUrl = url.parse(self.options.entryPoint, true);

    if (operation === 'logout') {
      if (self.options.logoutUrl) {
        targetUrl = url.parse(self.options.logoutUrl, true);
      }
    ...
    // We are deflating without providing zlib headers.
    // Some IdPs (simplesamlphp) cannot accept a payload any other way.
    // This optional URL param helps inform some IdPs (e.g. OneLogin) that this is the
    // compressed format we are using. Other IdPs have a setting to expect compression.
    ...
    targetUrl.query = extend(targetUrl.query, samlMessage);
    targetUrl.search = null;

    callback(null, url.format(targetUrl));
  });
};
...
SAML.prototype.processValidlySignedAssertion = function(xml, inResponseTo, callback) {
  ...
  var nowMs = this.options.nowInMs();
  var profile = {};
  var assertion;
  var parser = new xml2js.Parser(parserConfig);
  ...
  .then(function(doc) {
    ...
    if (subjectConfirmation) {
      if (confirmData && confirmData.$) {
        ...
        // If we have a max age, check if it is more restrictive
        var maxLimitMs = processMaxAgeAssertionTime(
            self.options.maxAssertionAgeMs, subjectNotOnOrAfter, assertion.$.IssueInstant);

        var subjErr = self.checkTimestampsValidityError(
              nowMs, subjectNotBefore, subjectNotOnOrAfter, maxLimitMs);
        if (subjErr) {
          throw subjErr;
        }
      }
    ...
    if (self.options.recipient) {
      var recipient = confirmData && confirmData.$ ?
        confirmData.$.Recipient : null;
      if (recipient !== self.options.recipient) {
  ...
  .then(function(){
    ...
    if(conditions && conditions.$) {
      var maxLimitMs = processMaxAgeAssertionTime(
              self.options.maxAssertionAgeMs, conditions.$.NotOnOrAfter, assertion.$.IssueInstant);
      var conErr = self.checkTimestampsValidityError(
          nowMs, conditions.$.NotBefore, conditions.$.NotOnOrAfter, maxLimitMs);
      if(conErr)
        throw conErr;
    }

    if (self.options.audience) {
      var audienceRestriction = conditions && conditions.AudienceRestriction ?
        conditions.AudienceRestriction[0] : null;
      var audience = audienceRestriction && audienceRestriction.Audience ?
        audienceRestriction.Audience[0] : null;
      if (audience !== self.options.audience) {
    ...
    profile.getAssertionDoc = function() { return assertion; };
...
SAML.prototype.checkTimestampsValidityError = function(nowMs, notBefore, notOnOrAfter, maxTimeLimitMs) {
  var self = this;
  ...
  // If we've imposed a max age restriction, we should check if we are stricter.
  if (maxTimeLimitMs) {
    if (nowMs - self.options.acceptedClockSkewMs >= maxTimeLimitMs) {
      return new Error('SAML assertion expired');
    }
  }
...
/**
 * Process max age assertion and use it if it is more restrictive than the age
 * assertion received in the SAML response.
 ...
 */
function processMaxAgeAssertionTime(maxAssertionAgeMs, notOnOrAfter, issueInstant) {
  var notOnOrAfterMs = Date.parse(notOnOrAfter);
  if (maxAssertionAgeMs === 0) {
    return notOnOrAfterMs;
  }

  var maxAssertionTimeMs = Date.parse(issueInstant) + maxAssertionAgeMs;
  return ((maxAssertionTimeMs < notOnOrAfterMs) ? maxAssertionTimeMs
      : notOnOrAfterMs);
}
