...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, isHttpPostBinding, callback) {
  ...
  (async () => {
    if(this.options.validateInResponseTo) {
      var saveFn = promisify(this.cacheProvider.save).bind(this.cacheProvider);
...
SAML.prototype.generateLogoutRequest = async function (req) {
  var id = "_" + this.generateUniqueID();
  var instant = this.generateInstant();
...
SAML.prototype.certsToCheck = async function () {
...
SAML.prototype.validatePostResponse = function (container, callback) {
  ...
  .then(async certs => {
    ...
    var doc2 = await parser.parseStringPromise(xml);
    var response = doc2.Response;
    if (response) {
      ...
      if (!assertion) {
        ...
        var logoutResponse = doc2.LogoutResponse;
        if (logoutResponse){
  ...
  .catch(async err => {
    ...
    if (this.options.validateInResponseTo) {
      var removeFn = promisify(this.cacheProvider.remove).bind(this.cacheProvider);
...
SAML.prototype.validateInResponseTo = async function (inResponseTo) {
  if (this.options.validateInResponseTo) {
    if (inResponseTo) {
...
SAML.prototype.getNameID = function(self, doc, callback) {
  ...
  if (encryptedIds.length === 1) {
    ...
    var decryptFn = promisify(xmlenc.decrypt).bind(xmlenc);
