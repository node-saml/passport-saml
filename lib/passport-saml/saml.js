...
SAML.prototype.signRequest = function (samlMessage) {
  ...
  samlMessage.Signature = signer.sign(this.options.providerPrivateKey, 'base64');
};
...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, callback) {
  ...
  .then(function() {
    ...
    if (self.options.providerPrivateKey && self.options.authnRequestBinding === 'HTTP-POST') {
      try {
        var samlMessage = {};
        ...
        signedXml.addReference("/samlp:AuthnRequest",
                               [
                                 'http://www.w3.org/2000/09/xmldsig#enveloped-signature',
                                 'http://www.w3.org/2001/10/xml-exc-c14n#'
                               ], 'http://www.w3.org/2001/04/xmlenc#sha256');
        signedXml.signingKey = self.options.providerPrivateKey;
        if (self.options.providerCert) {
          signedXml.keyInfoProvider = {
            getKeyInfo: function (key, prefix) {
              // TODO: figure out why signing key is passed in - keyinfo
              // includes certificate only

              prefix = prefix || '';
	      prefix = prefix ? prefix + ':' : prefix;

              let [,...cert] = self.options.providerCert.split('\n');
              cert = cert.splice(0, cert.length - 2);

              var output = "<" + prefix + "X509Data>";
              output += "<" + prefix + "X509Certificate>";
              output += cert.join('');
              output += "</" + prefix + "X509Certificate>";
              output += "</" + prefix + "X509Data>";

              return output;
            },
            getKey: function (keyInfo) {
              return self.options.providerCert;
            }
          };
        }

        signedXml.computeSignature(samlMessage.toSign, { prefix: 'ds' });
    ...
    return callback(null, generatedRequest);
  })
  .fail(function(err) {
    callback(err);
  })
...
SAML.prototype.generateLogoutResponse = function (req, logoutRequest) {
  ...
  var self = this;
  if (self.options.providerPrivateKey) {
    try {
      var samlMessage = {};
      samlMessage.toSign = generatedRequest;
      switch (self.options.signatureAlgorithm) {
      ...
      var signedXml = new xmlCrypto.SignedXml();
      signedXml.signatureAlgorithm = samlMessage.SigAlg;
      signedXml.addReference("/samlp:LogoutResponse",
                             [
                               'http://www.w3.org/2000/09/xmldsig#enveloped-signature',
                               'http://www.w3.org/2001/10/xml-exc-c14n#'
                             ], 'http://www.w3.org/2001/04/xmlenc#sha256');
...
SAML.prototype.requestToUrl = function (request, response, operation, additionalParameters, callback) {
  ...
  function requestToUrlHelper(err, buffer) {
    ...
    // Log request/response for debugging purposes
...
SAML.prototype.generateServiceProviderMetadata = function( decryptionCert, signingCert ) {
  ...
  if(this.options.providerPrivateKey){
    if(!signingCert){
      throw new Error(
  ...
  if(this.options.decryptionPvk || this.options.providerPrivateKey){
