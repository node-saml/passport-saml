...
SAML.prototype.generateLogoutRequest = function(req, callback) {
  var id = "_" + this.generateUniqueID();
  var instant = this.generateInstant();

  if (req.user && req.user.nameIDFormat && req.user.nameID) {
    var request = {
      'samlp:LogoutRequest': {
        '@xmlns:samlp': 'urn:oasis:names:tc:SAML:2.0:protocol',
        '@xmlns:saml': 'urn:oasis:names:tc:SAML:2.0:assertion',
    ...
    callback(new Error('Invalid req.user passed to logout'), null);
  }
};
...
SAML.prototype.getLogoutUrl = function(req, callback) {
  var self = this;
  self.generateLogoutRequest(req, function(err, request) {
    if (err)
      return callback(err);
  ...
  Q.fcall(function() {
    ...
    .then(function() {
      ...
      if (encryptedAssertions.length == 1) {
        ...
        var xmlencOptions = {
          key: self.options.decryptionPvk
        };
