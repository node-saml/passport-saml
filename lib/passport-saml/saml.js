...
SAML.prototype.validateSignature = function (fullXml, currentNode) {
  var self = this;
  var certs = [self.options.cert, self.options.alt_cert].filter(function (cert) {
    return (typeof cert == 'string' && cert.length);
  });
...
SAML.prototype.validatePostResponse = function (container, callback) {
  ...
  .then(function() {
    ...
    if (self.options.cert && self.validateSignature(xml, doc.documentElement)) {
      validSignature = true;
    }
    ...
    if (assertions.length == 1) {
      if (self.options.cert &&
          !validSignature &&
          !self.validateSignature(xml, assertions[0])) {
        throw new Error('Invalid signature');
      }
    ...
    if (encryptedAssertions.length == 1) {
      ...
      return Q.ninvoke(xmlenc, 'decrypt', encryptedAssertionXml, xmlencOptions)
        .then(function(decryptedXml) {
          ...
          if (self.options.cert &&
              !validSignature &&
              !self.validateSignature(decryptedXml, decryptedAssertions[0]))
            throw new Error('Invalid signature');
...
SAML.prototype.validatePostRequest = function (container, callback) {
  ...
  parser.parseString(xml, function (err, doc) {
    ...
    // Check if this document has a valid top-level signature
    if (self.options.cert && !self.validateSignature(xml, dom.documentElement)) {
