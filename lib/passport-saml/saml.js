...
SAML.prototype.initialize = function (options) {
  ...
  // xmlbuilder extra options
  if(!options.xmlbuilder) {
    options.xmlbuilder = {}; // no extra options
    ...
    /*
     ...
     @see https://github.com/oozcitak/xmlbuilder-js/blob/master/src/index.coffee
     */
  }
...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, reqOptions, callback) {
  ...
  Q.fcall(function() {
      ...
      .then(function(){
        ...
        var extensions = ((reqOptions && reqOptions.extensions) || []).concat( self.options.extensions || [] );

        if (extensions.length) {
          request['samlp:AuthnRequest']['samlp:Extensions'] = {};
          extensions.forEach( addExtension(request['samlp:AuthnRequest']['samlp:Extensions']) );
        }

        callback(null, createXML(request, self.options).end());
      })
      .fail(function(err){
        callback(err);
      })
...
//xmlbuilder helper function
function createXML( request, mainOptions ) {

  var opt = mainOptions.xmlbuilder;

  return xmlbuilder.create(request, opt.xmldec, opt.doctype, opt.options);
}

// add SAML 2 extensions info
function addExtension(extensionsTag) {
  //used in forEach
  return function(extensionInfo) {
    if(!extensionInfo.values) {
      return;
    }

    var xmlns = extensionInfo.xmlns.name;
    var urn = extensionInfo.xmlns.urn;

    Object.keys(extensionInfo.values).forEach(function(tag) {

      var entry = extensionsTag[xmlns+':'+tag] = {};

      var textTag = extensionInfo.cdata ?  '#cdata' : '#text';

      entry[textTag] = extensionInfo.values[tag];

      entry['@xmlns:'+xmlns] = urn;
...
SAML.prototype.generateLogoutRequest = function (req) {
  ...
  return createXML(request, this.options).end();
};
...
SAML.prototype.getAuthorizeUrl = function (req, reqOptions, callback) {
  //added for backward compatibility
  if (typeof reqOptions == 'function') {
    callback = reqOptions;
    reqOptions = {};
...
SAML.prototype.validateRedirectSignature = function (container, cert) {
  var self = this;
  return Q.fcall(function () {
    ...
    if (container.SAMLRequest) {
      signedQuery.SAMLRequest = container.SAMLRequest;
    }
    if (container.SAMLResponse) {
      signedQuery.SAMLResponse = container.SAMLResponse;
    }
    if (container.RelayState) {
      signedQuery.RelayState = container.RelayState;
    }
    if (container.SigAlg) {
      signedQuery.SigAlg = container.SigAlg;
    }

    var signer;
    switch(container.SigAlg) {
    ...
    signer.update(querystring.stringify(signedQuery));
    var sign = signer.sign(cert, 'base64');
    if(sign !== container.Signature){
      throw new Error('Unable to validate signature on query string.');
    }
    return true;
  });
};
...
SAML.prototype.validateGetResponse = function (container, callback) {
  var self = this;
  var xml = new Buffer(container.SAMLResponse, 'base64');
  zlib.inflateRaw(xml, function (err, data) {
    if(err){
      return callback(err);
    }
    var xml = new Buffer(data, 'base64').toString('utf8');
    var doc = new xmldom.DOMParser().parseFromString(xml);

    self.validateResponseInResponseTo(doc)
        .then(function (inResponseTo) {
          return Q.fcall(function () {
                  if(self.options.cert){
                      return self.validateRedirectSignature(container, self.options.cert);
                  }
                  return true;
              })
            .then(function () {
...
SAML.prototype.validateResponseInResponseTo = function (doc) {
  var self = this;
...
SAML.prototype.generateServiceProviderMetadata = function( decryptionCert ) {
  ...
  return createXML(metadata, this.options).end({ pretty: true, indent: '  ', newline: '\n' });
};
