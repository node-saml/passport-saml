...
SAML.prototype.initialize = function (options) {
  ...
  if(isNaN(options.acceptedClockSkewMs)){
      options.acceptedClockSkewMs = 0;   // default to no skew
  }
...
function processValidlySignedAssertion (xml, options, callback) {
  ...
  parser.parseString(xml, function (err, doc) {
    ...
    if(conditions && conditions.$ && options.acceptedClockSkewMs > -1) {

      var notBefore = conditions.$.NotBefore;
      var notOnOrAfter = conditions.$.NotOnOrAfter;
      ...
      if(notBefore){
        ...
        if(nowMs + options.acceptedClockSkewMs < notBeforeMs){
          return callback(new Error('SAML assertion not valid at the current time'), null, false);
        }
      }
      if(notOnOrAfter){
