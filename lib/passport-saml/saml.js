...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, callback) {
  ...
  .then(function(){
    ...
    //extensions tag
    if (self.options.extensions && Array.isArray(self.options.extensions)) {
      request['samlp:AuthnRequest']['samlp:Extensions'] = {};
      self.options.extensions.forEach( addExtension(request['samlp:AuthnRequest']['samlp:Extensions']) );
    }
