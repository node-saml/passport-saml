...
var SAML = function (options) {
  ...
  this.authnRequestIds = {};
  this.authnRequestIdList = [];

  // Expire old Authn request id's
  if(this.options.validateInResponseTo){
      setInterval(function(){

          var nowMs = new Date().getTime();
          self.authnRequestIdList.forEach(function(id){
              if(nowMs >= new Date(self.authnRequestIds[id]).getTime() + self.options.requestIdExpirationPeriodMs){
                  self.removeAuthnRequestId(id);
              }
          })
      }, this.options.requestIdExpirationPeriodMs);
  }
};
...
SAML.prototype.validatePostResponse = function (container, callback) {
  ...
  if(self.options.validateInResponseTo){
    if(inResponseTo && !self.doesAuthnRequestIdExist(inResponseTo)){
...
SAML.prototype.saveAuthnRequestId = function(id){

    if(!this.authnRequestIds[id])
    {
        this.authnRequestIds[id] = new Date().getTime();
        this.authnRequestIdList.push(id);
    }
};

SAML.prototype.doesAuthnRequestIdExist = function(id){
    if(this.authnRequestIds[id]) return true;

    return false;
};

SAML.prototype.removeAuthnRequestId = function(id){
    if(this.authnRequestIds[id])
    {
        delete this.authnRequestIds[id];
        this.authnRequestIdList.splice(this.authnRequestIdList.indexOf(id), 1);
    }
};
          ...
          if(subjectConfirmation){
              ...
              if(confirmData && confirmData.$){
                  ...
                  if(inResponseTo && subjectInResponseTo && subjectInResponseTo != inResponseTo){
                    self.removeAuthnRequestId(inResponseTo);
                    return callback(new Error('InResponseTo is not valid'), null, false);
                  }
                  else if(subjectInResponseTo)
                  {
                     if(!self.doesAuthnRequestIdExist(subjectInResponseTo)){
                       self.removeAuthnRequestId(inResponseTo);
                       return callback(new Error('InResponseTo is not valid'), null, false);
                     }
                  }
